...
export default function AddBooking({service}: ComponentProps) {
  ...
  const [professionalId, setProfessionalId] = useState<null | number>(professionals.length === 1 ? professionals[0].id : null);
  ...
  return (
    <div>
        ...
        <div>
          ...
          <div className="flex flex-col gap-2">
            {professionals.map((professional) => (
              <div
                key={professional.id}
                className={`flex gap-3 items-start border p-2 rounded-md  cursor-pointer ${professionalId === professional.id ? 'bg-blue-500 text-white ' : 'hover:bg-gray-100'}`}
                onClick={() => setProfessionalId(professional.id)}>
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center ${professionalId === professional.id ? 'bg-blue-700 text-white' : 'bg-gray-100'}`}>
      ...
      <button type="button" onClick={onSubmit} className="btn-primary"
              disabled={!selectedTime || !professionalId}>Reservar ahora
      </button>
